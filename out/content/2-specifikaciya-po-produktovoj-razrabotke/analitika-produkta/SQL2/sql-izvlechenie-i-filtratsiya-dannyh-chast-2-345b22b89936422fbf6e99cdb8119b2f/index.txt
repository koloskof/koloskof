...
11:Te9a,<h1>SQL: Извлечение и фильтрация данных (часть 2)</h1>
<hr>
<p><strong>Фильтрация строк с пустым значением IS NULL</strong></p>
...
<p><strong>Фильтрация строк с не пустым значением IS NOT NULL</strong></p>
<p>Структура запроса:</p>
<pre><code class="language-sql">SELECT *
FROM table_name
...
<p><strong>Фильтрация строк, присутствующих в списке IN()</strong></p>
<p>Структура запроса:</p>
<pre><code class="language-sql">SELECT *
FROM table_name
...
<p><strong>Фильтрация строк, отсутствующих в списке NOT IN()</strong></p>
<p>Структура запроса:</p>
<pre><code class="language-sql">SELECT *
FROM table_name
...
<p><strong>Фильтрация строк c несколькими условиями AND</strong></p>
<p>Структура запроса:</p>
<pre><code class="language-sql">SELECT *
FROM table_name
...
<p><strong>Фильтрация строк c несколькими условиями OR</strong></p>
<p>Структура запроса:</p>
<pre><code class="language-sql">SELECT *
FROM table_name
...
<p><strong>Фильтрация строк c N условиями AND и OR</strong></p>
<p>Структура запроса:</p>
<pre><code class="language-sql">SELECT *
FROM table_name
WHERE condition_1
...
<hr>
<p>Важно заметить, что запросы по прошедшему материалу не меняют никоим образом саму базу данных и данные в ней. Дополнительные столбцы у нас появляются в результате, сама таблица в базе данных остается такой же как и была.</p>
<p>Пояснения нам нужны для того, чтобы давать подсказки другим аналитикам и самому себе в будущем для понимания запроса. Также с помощью комментариев можно отметить части запроса, которые не должны быть выполнены.</p>
<p>Пример:</p>
<pre><code class="language-sql">-- it’s a comment
SELECT * 
/* again a comment
still a comment*/ 
FROM listings
